myApp.controller("modelController",function($scope,modelService,context) {
				...
				$scope.modelList = data.data;
				 ...
				 for (var i = 0; i < $scope.modelList.length; i++) {
					 ...
					 var tbody = '<tr class="text-c">' +
							         ...
							         '<a style="text-decoration:none" class="ml-5" onClick="publishModel(' + item.id + ')"  href="javascript:;" title="发布模型"><i class="Hui-iconfont">&#xe603;</i></a>' +
							         '<a style="text-decoration:none" class="ml-5" onClick="deleteModel(' + item.id + ')"  href="javascript:;" title="删除模型"><i class="Hui-iconfont">&#xe609;</i></a>' +
							         '</td>' +								
							      '</tr>';
...
function publishModel(id) {
	$.ajax({  
	    url : routers.ModelAPI + id + "?token=" + sessionStorage.token,  
	    type : 'PUT',  
	    processData : false,  
	    /**   
